 ...
 *     Vite app picks them up via `import.meta.env`.
 *
 * IMPORTANT: this writes a private key to disk. `.env.local` is
 * already covered by `.gitignore`. The wallet only ever holds testnet
 * / devnet "play" SOL — never real funds — so leaking the key is not
 * a financial risk.
 */
...
function writeEnv(payer, cluster) {
  ...
  const body = [
    '# Generated by scripts/setupSolanaWallet.mjs — DO NOT COMMIT',
    '# Shared Solana payer wallet used by every player to post their',
    '# playthrough summary on chain via the SPL Memo program.',
