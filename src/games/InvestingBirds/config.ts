 ...
 *
 * **Bonds** are the exception: `low === high` so the slice always earns that
 * fixed return regardless of clears.
 */
export const SLICE_RETURN_BOUNDS: Record<LevelType, { low: number; high: number }> = {
  bonds: { low: 0.04, high: 0.04 },
  etfs: { low: -0.2, high: 0.2 },
  stocks: { low: -0.5, high: 0.5 },
  crypto: { low: -1, high: 1 },
};
...
/** One-line range label for HUD / copy, e.g. "−20% to +20%" or "+4% (fixed)". */
export function sliceReturnRangeLabel(type: LevelType): string {
  const { low, high } = SLICE_RETURN_BOUNDS[type];
  if (low === high) {
    const pct = Math.round(Math.abs(low * 100));
    const sign = low >= 0 ? '+' : '−';
    return `${sign}${pct}% (fixed)`;
  }
...
export const CATEGORY_META: Record<LevelType, { label: string; accent: string; accentDark: string; blurb: string }> = {
  stocks: {
    ...
    accent: '#DC2626',
    accentDark: '#7f1d1d',
    blurb: 'High risk, high reward. Return ramps from −50% to +50% as you clear the tower.',
  },
  etfs: {
    label: 'ETFs',
    accent: '#3B82F6',
    accentDark: '#1e3a8a',
    blurb:
      'Medium risk. Five mini-towers with randomized blocks from easy breaks to stone-heavy; return ramps −20% to +20% as you clear them.',
  },
  bonds: {
    label: 'Bonds',
    accent: '#22C55E',
    accentDark: '#14532d',
    blurb: 'Low risk and predictable. Always +4% on that slice — no ramp with clears.',
  },
  crypto: {
    label: 'Crypto',
    accent: '#F59E0B',
    accentDark: '#78350f',
    blurb:
      'Wild swings. Tall narrow tower — mostly ice, a few stone anchors. One bird to clear it; return ramps −100% to +100%.',
  },
};
...
/** Demo fallback allocation used when the host does not provide Box data. */
export const DEMO_ALLOCATION: Allocation = {
  stocks: 25,
  etfs: 35,
  bonds: 25,
  crypto: 15,
...
/** Shots per round — only ETFs scale with share; all other categories use 1 bird. */
export function birdsForLevelType(type: LevelType, share: number): number {
  if (type === 'etfs') return birdsFromShare(share);
  return 1;
}
